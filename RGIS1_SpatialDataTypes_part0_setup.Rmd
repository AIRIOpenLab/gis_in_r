...
* It's best the everyone works with the same version of R to avoid problems, so please start by updating your installation of R to 3.5.2 ("Eggshell Igloo"). [Installers for R 3.5.2 can be found here.](http://cran.cnr.berkeley.edu/)
...
1. Install the `libgdal-dev` and `libproj-dev` packages and all their dependencies

2. Test that the installation is correct by typing `gdal-config`

3. run in the R console: `install.packages(c("sp", "raster", "rgdal"))`

__On Mac:__

1. Download [GDAL complete](http://www.kyngchaos.com/files/software/frameworks/GDAL_Complete-2.3.dmg)

2. Doubleclick and install the `.dmg` file as you are used to on a Mac

3. Make sure you have R Version 3.5.2 installed -- if not update it.

4. Download the [rgdal](https://cran.r-project.org/bin/macosx/el-capitan/contrib/3.5/rgdal_1.3-6.tgz) package from CRAN.

5. Place the downloaded `rgdal_1.3-6.tgz` in your Desktop folder

7. Run `install.packages("~/Desktop/rgdal1.3-6.tgz", repos=NULL)`
